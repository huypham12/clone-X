...
const MYID = new ObjectId('685678bf019ad1132c3663b2')
...
const checkAndCreateHashtags = async (hashtags: string[]) => {
  ...
  return hashtagDocuemts.filter((hashtag) => hashtag).map((hashtag) => (hashtag as WithId<Hashtag>)._id)
}
...
const insertTweet = async (user_id: ObjectId, body: TweetReqBody) => {
  ...
  const result = await databaseService.tweets.insertOne(
    new Tweet({
      ...
      hashtags,
      mentions: (body.mentions ?? []).map((id) => new ObjectId(id)),
      medias: body.medias ?? [],
...
insertMultipleUsers(users).then((ids) => {
  followMultipleUsers(MYID, ids).catch((err) => {
