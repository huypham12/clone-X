...
export const encodeHLSWithMultipleVideoStreams = async (inputPath: string) => {
  try {
    ...
    const hlsDir = path.join(parent_folder, videoName)

    // Create HLS directory if it doesn't exist
    if (!fs.existsSync(hlsDir)) {
      console.log('Creating HLS directory:', hlsDir)
      fs.mkdirSync(hlsDir, { recursive: true })
    }

    const outputSegmentPath = path.join(hlsDir, 'v%v/fileSequence%d.ts')
    const outputPath = path.join(hlsDir, 'v%v/prog_index.m3u8')

    // Create variant directories
    const variants = ['0', '1', '2'] // For 720p, 1080p, and 1440p/original
    for (const variant of variants) {
      const variantDir = path.join(hlsDir, `v${variant}`)
      if (!fs.existsSync(variantDir)) {
        console.log('Creating variant directory:', variantDir)
    ...
    console.log('Starting video encoding with parameters:', {
      inputPath,
      outputPath,
      outputSegmentPath,
      resolution,
    ...
    if (!fs.existsSync(masterPath)) {
      throw new Error(`Master playlist not created: ${masterPath}`)
    }

    // Log the content of master playlist for debugging
    const masterContent = fs.readFileSync(masterPath, 'utf-8')
    console.log('Master playlist content:', masterContent)

    return true
  } catch (error) {
    console.error('Error in encodeHLSWithMultipleVideoStreams:', error)
    // Clean up on error
    try {
      const videoName = path.basename(inputPath, path.extname(inputPath))
      const hlsDir = path.join(path.dirname(inputPath), videoName)
      if (fs.existsSync(hlsDir)) {
        fs.rmSync(hlsDir, { recursive: true, force: true })
      }
    } catch (cleanupError) {
      console.error('Error cleaning up after failed encoding:', cleanupError)
    }
    throw error
  }
}
